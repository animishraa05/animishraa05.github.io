...
![Non-Contiguous-Allocation](images/non-contiguous-allocation.webp)

- [[paging|Paging]]: It breaks processes into equal-sized pages and maps them to memory frames, mainly to eliminate external fragmentation.
...
- Segmented [[paging|Paging]]: A hybrid technique that first divides processes into logical segments and then splits each segment into fixed-size pages, combining the benefits of segmentation and [[paging]].
...
> Note: [[paging|Paging]] avoids external fragmentation but may suffer from internal fragmentation.
...
Example with [[paging|Paging]]: If frame size = 2KB, processes P1 and P2 (2 pages each) can be stored alternately across free frames in memory.

![Non_contiguous_allocation_3](images/non_contiguous_allocation_3.webp)
...
- Possibility of Page Faults (in [[paging|Paging]]-based systems) : Since data is scattered, a needed page may not be in memory, causing delays.