...
```dot
digraph user_mode_vs_kernel_mode {
  rankdir=LR
  node [shape=box style=filled fillcolor="#f0f4ff" fontname="Helvetica"]
  A [label="User Mode Vs Kernel \nInput"]
  B [label="User Mode Vs Kernel \nCore Mechanism"]
  C [label="User Mode Vs Kernel \nOutput"]
  A -> B [label="triggers"]
  B -> C [label="produces"]
}
```
...
```dot
graph semantic_user_mode_vs_kernel_mode {
  layout=neato
  node [shape=ellipse fontname="Helvetica" fontsize=11 style=filled]
  THIS [label="User Mode Vs Kernel " fillcolor="#ffd700" fontsize=13 style="filled,bold"]
