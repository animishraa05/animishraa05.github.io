...
- [[cache-memory|Cache Memory]]: High-speed memory that speeds up the CPU. It’s fast but expensive.
- Primary Memory (Main Memory): Includes RAM (volatile) and ROM (non-volatile). It stores current data.
- [[secondary-memory|Secondary Memory]]: Non-volatile memory used for permanent storage (e.g., hard drives, SSDs).

![Types of Memory](images_types-of-memory.png)
...
![What is RAM (Random Access Memory)](images_what-is-ram-random-access-memory.webp)
...
- SRAM is used as the [[cache-memory|Cache memory]] inside the computer.
...
| Usage | Used in [[cache-memory|cache memory]] for quick access | Used in main memory for large data storage |
